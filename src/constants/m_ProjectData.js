export const m_ProjectData = [
    {
        id: 1,
        title: "Axis:Save",
        description: "<strong>■ 本作における制御設計の適用</strong>\n本作では「状態競合の防止」「処理順序の保証」「責務分離」を重点的に適用しました。\n\nPlayerControllerは入力・物理を中心としつつ、HP管理や状態制御も内包しています。\n演出処理はCinematicManagerへ分離し、演出中は m_IsEventLocked により外部から操作ロックを行うことで、ロジックと演出の干渉を防止しています。\n\nまた、Update / FixedUpdate冒頭で死亡→被ダメージ硬直→イベントロックの順に早期returnする優先順位付きフラグ制御を採用しました。\nこれにより、複数状態が同時に成立することを防ぎ、状態の一貫性を保証しています。\n現状はフラグ制御ベースで安定性を確保しつつ、将来的には状態管理パターンへの移行により拡張性向上を検討しています。\n\n状態競合を避けつつ操作レスポンスを維持するため、攻撃中は移動を制限しダッシュのみ割り込み可能とするルールを設計しました。\n\n現状PlayerControllerに責務が集中しており、状態管理の拡張時に影響範囲が大きくなる構造であると認識しています。\n\n<strong>■ 物理干渉問題への対応</strong>\n演出中に入力・物理処理が競合し、状態破綻や意図しない位置遷移が発生する問題がありました。\n\n以下により解決：\n▸ FixedUpdate内での早期returnによる物理停止\n▸ Physics.IgnoreLayerCollisionによる当たり判定制御\n▸ UniTaskを用いたフレーム同期による状態復帰保証\n\nこれにより、演出中の入力混入・物理干渉による挙動破綻を防ぎ、演出の再現性を安定させました。\n\n<strong>■ カメラ演出設計</strong>\n▸ Lens.FieldOfViewを直接制御し、\n▸ elapsed / animationLength による補間係数算出を行い、\n▸ Mathf.Lerpにより時間的緩急を設計。\n\nCinemachineBlendを動的にCutへ変更することで、\n演出意図に応じたカメラ遷移制御を実装しました。\nこれにより、演出の緩急を制御し、プレイヤーの注目点を誘導しています。\n\n<strong>■ 技術選定理由</strong>\n・UniTaskを採用\n　→ 非同期処理を逐次実行に統一することで、状態遷移の順序をコード上で保証し、タイミング依存の不整合を防止\n　→ フレーム単位の同期制御ではなく、処理単位での順序保証が可能\n\n・IgnoreLayerCollisionを採用\n　→ Rigidbody停止では速度・状態リセット等の副作用が発生するため\n\n<strong>制作期間</strong> : 3ヶ月 &emsp; <strong>チーム人数</strong> : 個人制作 &emsp; <strong>プラットフォーム</strong> : PC / Windows",
        descriptionEn: "<strong>■ Application of Control Design</strong>\nIn this project, I focused on applying \"prevention of state conflicts,\" \"guaranteed processing order,\" and \"separation of responsibilities.\"\n\nPlayerController primarily handles input and physics, while also encompassing HP management and state control.\nPresentation processing is separated into CinematicManager, and during presentations, operation is locked externally via m_IsEventLocked to prevent interference between logic and presentation.\n\nAdditionally, I adopted priority-based flag control with early returns at the beginning of Update / FixedUpdate in the order of death → damage stagger → event lock.\nThis prevents multiple states from being active simultaneously, ensuring state consistency.\nCurrently, stability is ensured through flag-based control, while migration to a state management pattern is being considered for future extensibility improvements.\n\nTo avoid state conflicts while maintaining operation responsiveness, I designed a rule that restricts movement during attacks while allowing only dashes to interrupt.\n\nI recognize that responsibilities are currently concentrated in PlayerController, and the scope of impact becomes large when extending state management.\n\n<strong>■ Resolution of Physics Interference Issues</strong>\nA problem occurred where input and physics processing conflicted during presentations, causing state breakdown and unintended position transitions.\n\nSolved by:\n▸ Suspending physics via early return in FixedUpdate\n▸ Controlling hit detection using Physics.IgnoreLayerCollision\n▸ Guaranteeing state recovery through frame synchronization with UniTask\n\nThis prevented behavior breakdown due to input contamination and physics interference during presentations, stabilizing presentation reproducibility.\n\n<strong>■ Camera Presentation Design</strong>\n▸ Direct control of Lens.FieldOfView\n▸ Coefficient calculation via elapsed / animationLength\n▸ Designing temporal easing with Mathf.Lerp\n\nBy dynamically changing CinemachineBlend to Cut,\nI implemented camera transition control tailored to the presentation's intent.\nThis controls the pacing of presentations and guides the player's focal point.\n\n<strong>■ Technical Selection Rationale</strong>\n・Adopted UniTask\n　→ By unifying asynchronous processing into sequential execution, state transition order is guaranteed in code, preventing timing-dependent inconsistencies\n　→ Enables order guarantee at the processing unit level rather than frame-level synchronization control\n\n・Adopted IgnoreLayerCollision\n　→ Because stopping Rigidbody causes side effects such as velocity and state resets\n\n<strong>Duration</strong> : 3 months &emsp; <strong>Team</strong> : Solo &emsp; <strong>Platform</strong> : PC / Windows",
